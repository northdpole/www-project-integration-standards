...
def readSpreadsheet(url: str, cres_loc: str):
    ...
    try:
        ...
        logger.error("Error optining spreadsheet " + url)
        logger.error(ae)
...
def add_to_tree(cre_loc:str, alias:str):
...
def pushToGithub(cre_loc: str, apikey: str):
    g = git.Git()
    repo = git.Repo(os.path.join(os.path.dirname(os.path.realpath(__file__)), "../../"))
    g.checkout(commit_msg_base)    
    repo.remotes.origin.push(commit_msg_base)
    ...
    createPullRequest(apiToken=apikey, repo=remoteURL[0].replace("git@github.com:", "").replace(".git", ""),
                      title=commit_msg_base, srcBranch=commit_msg_base, targetBranch="master")
...
def main():
    ...
    with open(os.path.join(script_path, spreadsheets_file)) as sfile:
        ...
        for spreadsheet_url in urls:
            readSpreadsheet(spreadsheet_url['url'], cres_loc=cre_loc)
            add_to_tree(cre_loc, spreadsheet_url['alias'])
